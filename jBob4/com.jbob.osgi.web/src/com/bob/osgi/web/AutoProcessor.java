...
public class AutoProcessor {
	...
	**/
	public static final String AUTO_DEPLOY_DIR_PROPERY = "auto.deploy.dir";
	/**
	...
	**/
	public static final String AUTO_DEPLOY_ACTION_PROPERY = "auto.deploy.action";
	/**
	 * The property name used to specify auto-deploy start level.
	**/
	public static final String AUTO_DEPLOY_STARTLEVEL_PROPERY = "auto.deploy.startlevel";
	/**
	...
	**/
	public static final String AUTO_INSTALL_PROP = "auto.install";
	/**
	 * The property name prefix for the launcher's auto-start property.
	**/
	public static final String AUTO_START_PROP = "auto.start";
	...
	**/
	public static void process(Map<?,?> configMap, BundleContext context) {
		configMap = (configMap == null) ? new HashMap<Object,Object>() : configMap;
	...
	private static void processAutoDeploy(Map<?,?> configMap, BundleContext context) {
		...
		if (actionList.size() > 0) {
			...
			File[] bundleSubDirs = new File(autoDir).listFiles();

			final List<Bundle> startBundleList = new ArrayList<Bundle>();

			if (bundleSubDirs != null) {
				Arrays.sort(bundleSubDirs);
				Pattern pattern = Pattern.compile("\\d+");
				for (int j = 0; j < bundleSubDirs.length; j++) {
					if (bundleSubDirs[j].isDirectory() && pattern.matcher(bundleSubDirs[j].getName()).matches()) {
						
						int bundleLevel = Integer.parseInt(bundleSubDirs[j].getName());
						
						File[] files = bundleSubDirs[j].listFiles();
						...
						for (int i = 0; i < jarList.size(); i++) {
							...
							Bundle b = installedBundleMap.remove(((File) jarList.get(i)).toURI().toString());
							...
							try {
								...
								if ((b != null) && !isFragment(b)) {
									...
									sl.setBundleStartLevel(b, bundleLevel);
								}
			...
			if (actionList.contains(AUTO_DEPLOY_UNINSTALL_VALUE)) {
				for (Iterator<Map.Entry<String,Bundle>> it = installedBundleMap.entrySet().iterator(); it.hasNext();) {
					Map.Entry<String,Bundle> entry = it.next();
